...
class IssueBookCBView(View):
    def get(self, request, *args, **kwargs):
        ...
        bookavailable = Books.objects.filter(
            id=kwargs['book_id'], quantity__gt=0)
        try:
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            if bookavailable.count():
                ...
                if bookexist:
                    messages.warning(
                        request, "book already issue by you. one book cannot issue multiple at a times ")
                    return redirect('books')
                bookavailable = bookavailable.get()
                Transaction.objects.create(
                    book=bookavailable, issue_by=request.user)
                ...
                if transexist.exists():
                    messages.error(
                        request, 'opps you are already in waiting queue')
                    return redirect('mybooks')
                WaitingTransaction.objects.create(
                    book=book1, issue_by=request.user)
...
class MybookCBV(View):
    def get(self, request):
        allbook = Transaction.objects.filter(
            issue_by=request.user, return_date=None)
